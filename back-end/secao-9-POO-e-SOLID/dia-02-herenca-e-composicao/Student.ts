...
export default class Student extends Person {
  ...
  generateEnrollment(): string {
	...
	sumGrades(): number {
		const sumExams = this.examsGrades.reduce((acc, curr) => acc + curr, 0);
		const sumAssignments = this.assignmentsGrades.reduce((acc, curr) => acc + curr, 0);

		return sumExams + sumAssignments;
	}
